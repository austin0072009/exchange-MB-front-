...
export default {
	...
	created() {
		this.symbol = window.localStorage.getItem("currentCurrency") + '/USDT';//this.symbol = window.localStorage.getItem('symbol');

		 this.leftName = window.localStorage.getItem("legal_name") || "USDT";
		this.rightName = window.localStorage.getItem("currentCurrency") || "BTC";
		this.currencyId = 1;// this.currencyId = window.localStorage.getItem('legal_id');
		this.legalId = 3;//this.legalId = window.localStorage.getItem('currency_id');
	...
	methods: {
		connect(real) {
			...
			that.sockets.subscribe('daymarket', res => {
				eventBus.$emit('updateDaymarkey', res);
			});
		},
