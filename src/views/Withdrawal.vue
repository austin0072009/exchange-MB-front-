  ...
  <div>
    ...
    <mu-dialog width="360" transition="slide-bottom" fullscreen :open.sync="openFullscreen" class="bg000">
      ...
      <div style="padding: 0px 0;">
        <mu-list>
          <mu-divider v-for="(item,index) in allbis" v-if="index>0"></mu-divider>
          <mu-list-item button @click="ccharge(index)" :ripple="true"  v-for="(item,index) in allbis">
            <mu-list-item-title>{{item.name}}</mu-list-item-title>
            <mu-list-item-action>
              <mu-icon v-if="charge==item.name" color="primary" value=":icon-chenggong1"></mu-icon>
            </mu-list-item-action>
          </mu-list-item>
...
export default {
  ...
  data() {
    return {
      ...
      obj: {
		...
		allbis:[],
		cbis:["BTC","ETH","USDT","TRX","DOGE","XRP","BCH"],//这个是要显示的提币币种列表
    }
  },
  watch: {
   /* charge(val) {
  ...
  methods: {
	  ccharge:function(index){
		this.currency=this.allbis[index].id;
		this.charge=this.allbis[index].name;
		...
		this.$http({
		  ...
		  if (res.data.type === 'ok') {
			  ...
			  for(let i=0;i<allbis.length;i++){
				  if(this.isConArr(allbis[i].currency_name,this.cbis)){
					let o={
	...
	isConArr:function(val,arr){
		var arrII=0;
		for(arrII=0; arrII<arr.length; arrII++){
			if(arr[arrII] == val)
				return true;
		}
		return false;
    ...
    submit() {
      ...
      this.$http({
		...
		let mg=res.data.message
        if (res.data.type === 'ok') {
          that.$toast.success(that.$t("apitip."+mg));
          that.address = '';
          that.number = '';
		  that.openFullscreen=true;
        } else {
			if(mg.indexOf("提币金额范围为：")>0){
				let fw=mg.replace(that.charge+"提币金额范围为：","");
				that.$toast.error(that.charge+" "+that.$t("apitip.提币金额范围为：")+fw);
			}else{
				that.$toast.error(that.$t("apitip."+mg));
			}
